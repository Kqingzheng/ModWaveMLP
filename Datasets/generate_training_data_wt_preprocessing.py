...
def data_to_wt(data, type, level):
    ...
    for i in range(level):
        coeffs[-(i + 1)] = pywt.threshold(coeffs[-(i + 1)], value=threshold, mode="soft", substitute=0)
    ...
    for t in range(min_t, max_t):
        ...
        for type in wt_type_list:
            ...
            for i in range(x_t.shape[1]):
                data_wt.append(data_to_wt(x_t[:, i,0], type, level))
            data_arr = np.asarray(data_wt)
            data_arr = data_arr.transpose((1, 0))
            data_arr = np.expand_dims(data_arr, axis=-1)
            x_t_list.append(data_arr)
        x_t_wtdata = np.concatenate(x_t_list, axis=-1)     
...
def generate_train_val_test(args):
    ...
    for cat in ["train", "val", "test"]:
        ...
        np.savez_compressed(
            os.path.join(args.output_dir, "%s-history-%d-horizon-%d-day_in_week_wavelet_db_fam_level2_soft_preprocessing.npz" % (cat, args.history_length, args.horizon)),
            x=_x,
            y=_y,
