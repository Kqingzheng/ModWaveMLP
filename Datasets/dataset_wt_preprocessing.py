...
import pathlib
from Datasets.generate_training_data_wt_preprocessing import generate_train_val_test
...
class Dataset(object):
    def __init__(self, name='metr-la', horizon: int = 3, history_length: int = 3, path: str = 'data'):
        ...
        for category in ['train', 'val', 'test']:
            data_filename = os.path.join(dataset_parameters["output_dir"], category + f"-history-{self.history_length}-horizon-{self.horizon}-day_in_week_wavelet_db_fam_level2_soft_preprocessing.npz")
